...
class TrackFollowTask(NavigationTask):
    ...
    def extract_target_bbox_from_segmentation(self, segmentation_mask):
        """
        Extract 2D bounding box of TARGET_SEMANTIC_ID from segmentation mask.
        ...
        if num_cameras > 0:
            ...
            if torch.any(target_found):
                ...
                y_masked_min = torch.where(y_has_target, y_coords_expanded, torch.full_like(y_coords_expanded, float('inf')))
                y_masked_max = torch.where(y_has_target, y_coords_expanded, torch.full_like(y_coords_expanded, float('-inf')))
                x_masked_min = torch.where(x_has_target, x_coords_expanded, torch.full_like(x_coords_expanded, float('inf')))
                x_masked_max = torch.where(x_has_target, x_coords_expanded, torch.full_like(x_coords_expanded, float('-inf')))
    ...
    def save_segmentation_visualization(self, save_every_n_steps=1):
        ...
        if np.any(bbox_normalized != 0):
            ...
            # Add text label
            ax.text(x_min, y_min - 5, f'Target BB: [{x_min}, {y_min}, {x_max}, {y_max}]',
                   color='red', fontsize=10, weight='bold',
                   bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
        else:
            # No target found
            ax.text(width // 2, height // 2, 'No Target Detected',
                   color='red', fontsize=16, weight='bold', ha='center', va='center',
        ...
        if "depth_range_pixels" in self.obs_dict:
            ...
            if np.any(valid_mask):
                ...
                depth_normalized[valid_mask] = np.clip(
                    (depth_vis[valid_mask] - p1) / (p99 - p1 + 1e-6), 0, 1
                )
            ...
            logger.info(f"Saved depth visualization to {depth_save_path} "
                       f"(range: [{depth_min_m:.2f}, {depth_max_m:.2f}] m)")
